...
public class Jiggler : MonoBehaviour
{
    ...
    public float distanceFromKart = 10f;
    void Start(){
    ...
    void Update()
    {
        ...
        var dist = Vector3.Distance(this.transform.position, kart.transform.position);

        if (dist > distanceFromKart)
            scaleTime += dt * scaleFrequency;
        else
            scaleTime += dt * 4*scaleFrequency;
        if (jigPosition)
